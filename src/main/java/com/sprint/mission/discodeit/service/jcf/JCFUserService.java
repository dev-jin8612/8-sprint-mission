...
public class JCFUserService implements UserService {
    ...
    public void deleteUser(String name) {
        ...
        if (user != null) {
            System.out.println(user.getUserName() + "방을 삭제했습니다.");
            users.remove(user);
        } else {
            System.out.println("없거나 삭제된 방입니다.");
        }
    }
...
//        아무래도 단체용은 따로 하는게 좋을것 같다.
    ...
    // 방'들' 찾기
    @Override
    public void searchUserS(ArrayList<String> names) {
        // 여기서 여러번 부르는 것보다는
    ...
    public void searchUpdateUser() {
        ...
        users.forEach(u -> {
            if (u.getCreated() != u.getUpdated()) {
                System.out.println("업데이트된 방: " + u.getUserName());
                notNull.set(true);
            }
        });

        if(notNull.get() ==false){
            System.out.println("업데이트된 방가 없습니다.");
        }
    }
}